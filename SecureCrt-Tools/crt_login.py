...
def Main():
    ...
    # Check for custom csv_path passed from loader (global variable)
    global custom_csv_path  # Ensure it's recognized if not already in scope
    if 'custom_csv_path' in globals() and custom_csv_path and os.path.exists(custom_csv_path):
        csv_path = custom_csv_path
    else:
        # Original fixed path and fallback prompt
        csv_path = r"C:\workarea\credentials.csv"
